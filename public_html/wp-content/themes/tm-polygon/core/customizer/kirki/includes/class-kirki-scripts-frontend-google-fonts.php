...
class Kirki_Scripts_Frontend_Google_Fonts {
				...
				if ( in_array( $output['property'], array( 'font-family', 'font-weight', 'font-subset' ) ) ) {
					...
					if ( 'option' == Kirki::$config[ $config_id ]['option_type'] && '' != Kirki::$config[ $config_id ]['option_name'] ) {
						$settings = str_replace( array(
							']',
							Kirki::$config[ $config_id ]['option_name'] . '['
						), '', $field['settings'] );
					}
	...
	public function google_font() {
		...
		if ( $this->google_link() ) {
			$google_links = str_replace( '%3A', ':', $this->google_link() );
			$google_links = str_replace( '%257C', '|', $google_links );
			wp_enqueue_style( 'kirki_google_fonts', $google_links, array(), '1.0.0' );
		}
	}
