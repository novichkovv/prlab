...
function jm_post_like() {
	...
	if ( isset( $_POST['jm_post_like'] ) ) {
		...
		echo ( $post_like_count < 1 ) ? '' : ' ' . ( $post_like_count == 1 ? esc_html__( 'Like', 'tm-polygon' ) : esc_html__( 'Likes', 'tm-polygon' ) );
	}
...
function getPostLikeLink( $post_id ) {
	...
	$count      = ( empty( $like_count ) || $like_count == "0" ) ? 'Like' : sprintf( _n( '%s Like', '%s Likes', $like_count, 'tm-polygon' ), $like_count );
	$like_text  = 0;
	...
	$output = '<a href="#" class="jm-post-like' . esc_attr( $class ) . '" data-post_id="' . esc_attr( $post_id ) . '" title="' . esc_attr( $title ) . '">' . $heart . '&nbsp;' . $count . '</a>';
...
function show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php esc_html_e( 'You Like:', 'tm-polygon' ); ?></label></th>
			<td>
				<?php
				...
				if ( $count > 0 ) {
					...
					echo wp_kses( $like_list , wp_kses_allowed_html( 'post' ) );
				} else {
					echo "<p>" . esc_html_e( 'You don\'t like anything yet.', 'tm-polygon' ) . "</p>\n";
				} ?>
			</td>
		</tr>
	</table>
...
function frontEndUserLikes() {
	if ( is_user_logged_in() ) { // user is logged in
		...
		if ( $count > 0 ) {
			...
			$like_list .= "<aside>\n";
			$like_list .= "<h3>" . esc_html__( 'You Like:', 'tm-polygon' ) . "</h3>\n";
			$like_list .= "<ul>\n";
...
function jm_most_popular_today() {
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . esc_html_e( 'Today\'s Most Popular Posts', 'tm-polygon' ) . "</h3>\n";
		echo "<ul>\n";
...
function jm_most_popular_month() {
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . esc_html_e( 'This Month\'s Most Popular Posts', 'tm-polygon' ) . "</h3>\n";
		echo "<ul>\n";
		while ( $pop_posts->have_posts() ) {
			$pop_posts->the_post();
			echo "<li><a href='" . get_permalink( $post->ID ) . "'>" . get_the_title() . "</a></li>\n";
...
function jm_most_popular_week() {
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . esc_html_e( 'This Week\'s Most Popular Posts', 'tm-polygon' ) . "</h3>\n";
		echo "<ul>\n";
		while ( $pop_posts->have_posts() ) {
			$pop_posts->the_post();
			echo "<li><a href='" . get_permalink( $post->ID ) . "'>" . get_the_title() . "</a></li>\n";
...
function jm_most_popular() {
	global $post;
	echo "<aside>\n";
	echo "<h3>" . esc_html_e( 'Most Popular Posts', 'tm-polygon' ) . "</h3>\n";
	echo "<ul>\n";
	$args      = array(
		'post_type'      => array( 'post', 'enter-your-comma-separated-post-types-here' ),
		'meta_query'     => array(
